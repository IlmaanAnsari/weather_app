...
const forecast = (address,callback) => {
    ...
    request({url : url, json : true}, (error,response) => {
        if(error){
            ...
            callback(undefined,response.body.current.weather_descriptions+'It is currently ' +response.body.current.temperature+ ' degrees out and feels like ' + response.body.current.feelslike+'. There are '+response.body.current.precip+ '% of raining.')  ;  
        }
